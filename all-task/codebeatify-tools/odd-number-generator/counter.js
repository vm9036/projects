var _statcounter = function(_1) {
	...
	try {
		...
		if (document.currentScript && document.currentScript.src) {
			...
			if (_b) {
				try {
					if (new URL(_b.src).host.replace(/www\.|dev.[0-9]+\./, "") === "statcounter.com") {
						_c = _b.src
					} else {}
				} catch (ignore) {}
			}
		...
		function _sessionStorageSetConfig(key, _c5) {
			...
			try {
				_c6[key] = _c5;
				sessionStorage.setItem("statcounter_config", JSON.stringify(_c6));
				return true
			} catch (ignore) {
				return false
			}
		...
		var _c7 = function(_c8) {
			...
			if (!_c9 || !_c9.match(/(^on$|test$|test_[a-z]{12}$|wsdev$|^dev[0-9]*)/)) {
				return
			}
			if (isLegacyIE()) {
				return
		...
		_7.get_visitor_id = function() {
			...
			var _1b2 = false;
			try {
				_1b2 = getLocal("is_visitor_unique")
			} catch (e) {}
			if (_1b2 && _1b2.substring(0, 2) == "rx" && _1b2.split(".").length > 2 && _1b2.split(".")[2].length == 32) {
				return _1b2.split(".")[2]
			}
			return "x-no-visitor"
		};
		_7.get_session_num = function(_1b3) {
			var _1b4 = _sessionStorageGetConfig("sc_project_time_difference_" + _1b3);
			var _1b5 = false;
			if (!_1b4 && _23 !== "ntd") {
				_1b5 = _23
			} else {
			...
			if (_1b5 != false && _7._session_increment_calculated[_1b3] && _7._returning_values[_1b3].length == _22.length) {
				for (var i = 0; i < _22.length; i++) {
					if (_22[i] * 60 == parseInt(_1b5)) {
						return _7._returning_values[_1b3][i]
					}
				}
			}
			var _1b7 = "-" + _23 + "-" + _1b3 + "-" + _24 + "-" + _20;
			if (_1b4 === null) {
				return "x-no-session-num-99" + Math.round(1000 * Math.random()) + _1b7
			} else {
				if (!_7._session_increment_calculated[_1b3]) {
					return "x-no-session-num-98" + Math.round(1000 * Math.random()) + _1b7
				} else {
					if (_7._returning_values[_1b3].length !== _22.length) {
						return "x-no-session-num-97" + Math.round(1000 * Math.random()) + _1b7
					} else {
						if (_1b4 == false) {
							return "x-no-session-num-96" + Math.round(1000 * Math.random()) + _1b7
						}
					}
				}
			}
			return "x-no-session-num-95" + _1b4 + _1b7
		};
		_7.version = function() {
			return "c5bd59"
		};
		_7.get_tab_session = function() {
			var _1b8 = false;
			try {
				_1b8 = sessionStorage.getItem("statcounter_tab_session");
				if (!_1b8) {
					_1b8 = generate_uuid(8);
					try {
						sessionStorage.setItem("statcounter_tab_session", _1b8)
					} catch (e) {
						_1b8 = false
					}
				}
		...
		_7.record = function(_1b9, _1ba) {
			if (_1ba === undefined) {
				_1ba = "on"
			} else {}
			if (_1b9 === undefined || _1b9 === "on" || _1b9 === "dev") {
				if (_2 !== false) {
				...
				if (parseInt(_1b9, 10) + "" == _1b9) {
					_1b9 = parseInt(_1b9, 10)
				} else {
					return
				}
			}
			_sessionStorageSetConfig("record_" + _1b9, _1ba);
			if (!_sessionStorageGetConfig("sc_project_time_difference_" + _1b9)) {
				_sessionStorageSetConfig("sc_project_time_difference_" + _1b9, 1800)
			}
			_c7(_1b9)
		};
		...
		if (_8 == 1) {
			if (_f > 0) {
				var _1bb = [];
				_1bb.push.apply(_1bb, _28.getElementsByTagName("frame"));
				_1bb.push.apply(_1bb, _28.getElementsByTagName("iframe"));
				while (_1bb.length) {
					var _1bc = _1bb.pop(0);
					try {
						var fdoc = _1bc.contentDocument;
						initiate_click_detection(fdoc);
						_1bb.push.apply(_1bb, fdoc.getElementsByTagName("frame"));
						_1bb.push.apply(_1bb, fdoc.getElementsByTagName("iframe"))
					} catch (ignore) {}
				}
				initiate_click_detection(_28)
			}
			try {
				var _1be = _28.getElementsByTagName("title");
				if (_1be.length) {
					var _1bf = _28.title;
					var _1c0 = _28.location.href.split("#")[0];
					var _1c1 = new MutationObserver(function() {
						var _1c2 = _28.location.href.split("#")[0];
						if (_28.title != _1bf && _1c2 != _1c0) {
							for (var _1c3 in _7._security_codes) {
								var _1c4 = parseInt(_1c3, 10);
								if (_ff[_1c4] === undefined || (new Date().getTime() - _ff[_1c4] > 1000)) {
									setTimeout(function(_1c5) {
										if (_ff[_1c5] === undefined || (new Date().getTime() - _ff[_1c5] > 1000)) {
											if (_27.sc_top_reg !== undefined) {
												_27.sc_top_reg[_1c5] = undefined
											}
											_7.record_pageview(_1c5)
										}
									}, 200, _1c4)
								}
							}
		...
		if (_2 != false && is_admin_project(_2)) {
			...
			var _1c7 = "";
			for (var prop in e) {
				_1c7 += "property: " + prop + " value: [" + e[prop] + "]\n"
			}
