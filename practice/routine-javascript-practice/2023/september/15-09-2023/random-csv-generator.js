function generateRandomCSV() {
  ...
  stringType = document.getElementsByName("random");
  ...
  let length = Number(document.getElementById("max-string-length").value);
  console.log("length :->"+length);

  let quotes = document.getElementById("quote").checked;
  console.log("quotes :-> "+quotes);

  let alphabet = document.getElementById("string-alphabet").value;
  ...
  if (isNaN(cols) || cols <= 0) {
    this.output.showNegativeBadge(
      "Can't generate",
      '"' + cols + '" is not a valid column count.'
    );
    return "";
  }
  ...
  if (!english) {
    if (isNaN(length) || length < 0) {
      this.output.showNegativeBadge(
        "Can't generate",
        '"' + length + '" is not a valid string length.'
      );
      return "";
    }
    if (!alphabet) {
      this.output.showNegativeBadge(
        "Can't generate",
        "Invalid random string alphabet."
      );
      return "";
    }
  }
  ...
  for (var i = 0; i < cols; i++) {
    headers.push(randomString(opts));
  }
  var json = [];
  for (var row = 0; row < rows; row++) {
    var line = {};
    for (var col = 0; col < headers.length; col++) {
      line[headers[col]] = randomString(opts);
    }
    json.push(line);
  }
  var config = {
  ...
  document.getElementById("generatedRandomCSVTextArea").value = csv;
}
...
function copyGeneratedRandomCSV(){
  copyToClipboard(document.getElementById("generatedRandomCSVTextArea").value);
  document.getElementById("generatedRandomCSVTextArea").select();
}
function downloadGeneratedData(){
  var blob = new Blob([ "" + document.getElementById("generatedRandomCSVTextArea").value + "" ], {
