function word_wrap(str, options) {
    ...
    var trimLines = options.trimLines || false;
    ...
    for (var i = 0; i < inputLines.length; i++) {
        if (inputLines[i].trim().length == 0) {
            ...
            if (options.breakWords !== true) {
                outputLines.push(textToWordChunks(inputLines[i], width));
            } else {
                outputLines.push(textToChunks(inputLines[i], width, trimLines));
            }
        }
    }
    return outputLines.join('\n');
}
;function textToWordChunks(str, width) {
    ...
    for (var i = 0; i < words.length; i++) {
        var word = words[i];
        var wordsChars = splitter.splitGraphemes(word);
        if (wordsChars.length > width && line.length == 0) {
            lines.push(wordsChars.join(''));
            continue;
        }
        if (line.length + wordsChars.length <= width) {
            if (word.trim().length != 0 || line.length != 0) {
                line = line.concat(wordsChars);
            }
        } else {
...
function textToChunks(str, width, trimLines) {
    var splitter = new GraphemeSplitter();
    var chars = splitter.splitGraphemes(str);
    var chunks = [];
    for (var i = 0; i < chars.length; i += width) {
        if (trimLines) {
            for (var j = 0; ; j++) {
                if (chars[i + j].trim().length > 0)
                    break;
            }
            i += j;
        }
        var chunk = chars.slice(i, i + width);
...
function longest_line(lines) {
    ...
    for (var i = 0; i < lines.length; i++) {
        var lineLen = splitter.splitGraphemes(lines[i]).length;
        if (lineLen > longest)
            longest = lineLen;
    }
    return longest;
}
function justify_text(str, options) {
    var opts = Object.assign({
        random: random,
        width: 55
    }, options);
    lastRandomIndex = 0;
    str = word_wrap(str, opts);
    var lines = str.split(/\r?\n/);
    var longestLength = longest_line(lines);
...
;function toMaxLength(str, max, opts) {
    var splitter = new GraphemeSplitter();
    var len = splitter.splitGraphemes(str).length;
    var diff = max - len;
    var words = str.split(' ');
    var n = words.length;
    while (diff-- > 0) {
...
function random(max) {
    var randomTable = [0.14975434898516649, 0.48920102670502064, 0.5460856891554375, 0.843156677246687, 0.5339584048702419, 0.6557222579529927, 0.025277961886063904, 0.7906919328816282, 0.39547316726978843, 0.8046447313481258, 0.40539968121122616, 0.3240593946801942, 0.04700841115323562];
