...
function getChainData(tool) {
    var getEncoded = function (tool) {
        var overridden = JSON.parse(tool.sides.getAttribute("data-override-options") || "{}");
        var query = [];
...
function createTool(name, extra) {
    ...
    if (simple) {
        ...
        tool = new BinaryTool(bridge.converter, opts, extra);
    }
...
/* 
    Binary Tool is a Text Tool designed to work
    with binary values and file examples.
*/
function BinaryTool (bridge, config, extra) {
    var tool = new Tool({
        converter: bridge,
        sides: extra.sides,
        examples: extra.examples,
    ...
    function downloadBlob (source, callback) {
    ...
    function setInputValue (value) {
        var input = this.input.element.querySelector(".binary-textarea");
        return BinaryArea.setValue(input, value);
    }

    function getInputValue () {
        var input = this.input.element.querySelector(".binary-textarea");
        return BinaryArea.getValue(input);
    }

    function setOutputValue (value) {
        var output = this.output.element.querySelector(".binary-textarea");
        return BinaryArea.setValue(output, value);
    }

    function getOutputValue () {
        var output = this.output.element.querySelector(".binary-textarea");
        return BinaryArea.getValue(output);
    }

    function download (side, format) {
        var tool = this;
        ...
        if (tool[side].element) {
            ...
            var blob = new Blob([tool[side].getValue()], {type: "application/octet-stream;charset=utf-8"});
            saveAs(blob, "{0}-{1}.{2}".format(side, tool.siteName, format));
        }
    }


    function fileExample (example) {
        var tool = this;
        var sample = example.querySelector(".input-sample");

        if (sample.classList.contains("text-sample")) {
            var text = sample.querySelector("span");
            var value = (text ? text.textContent : "");

            tool.input.setValue(value);
            tool.options.set(tool.options.get(example));
            tool.convert(Trigger.EXAMPLE);
        }

        else if (sample.classList.contains("file-sample")) {
            var source = sample.getAttribute("data-src");

            tool.input.showStatus("loading example...");

            downloadBlob(source, function(blob) {
                ...
                reader.onload = function(event) {
                    ...
                    for (var i = 0; i < typed.length; i++) {
                        text += String.fromCharCode(typed[i]);
                    }
    ...
    if (config.input && config.input.binary === true) {
        var area = tool.input.element.querySelector(".binary-textarea");
        BinaryArea.initializeArea(area);

        tool.input.setValue = setInputValue.bind(tool);
        tool.input.getValue = getInputValue.bind(tool);
        tool.input.setBinaryDisplayMode = area.binaryAreaSetMode;
        tool.setExample = fileExample.bind(tool);

        BinaryArea.onChange(area, function(){
            tool.convert(Trigger.KEYPRESS);
        })

        tool.input.download = download.bind(tool, "input", config.input.download || "bin");

        var content = area.querySelector(".binary-textarea-content");
        
        tool.input.toClipboard = function(){
            content.focus();
            content.select();
            try {
                ...
                if (copied) {
                    ...
                    var nulCharacterAt = tool.input.getValue().indexOf(String.fromCharCode(0));
                    var isOnWindows = window.navigator.userAgent.indexOf("Windows") !== -1;

                    if (nulCharacterAt !== -1 && isOnWindows) {
                        tool.input.showWarningBadge("Binary data in clipboard", "The binary data you copied contains a NUL character at position {0}. The rest of the data might have been lost.".format(nulCharacterAt));
                    }
                }
                else tool.input.showStatus("can't copy");
    ...
    if (config.output && config.output.binary === true) {
        var area = tool.output.element.querySelector(".binary-textarea");
        BinaryArea.initializeArea(area);

        tool.output.setValue = setOutputValue.bind(tool);
        tool.output.getValue = getOutputValue.bind(tool);
        tool.output.setBinaryDisplayMode = area.binaryAreaSetMode;

        tool.output.download = download.bind(tool, "output", config.output.download || "bin");

        var content = area.querySelector(".binary-textarea-content");

        tool.output.toClipboard = function(){
            content.focus();
            content.select();
            try {
                var copied = document.execCommand("copy");
                if (copied) {
                    tool.output.showStatus("copied");

                    var nulCharacterAt = tool.output.getValue().indexOf(String.fromCharCode(0));
                    var isOnWindows = window.navigator.userAgent.indexOf("Windows") !== -1;

                    if (nulCharacterAt !== -1 && isOnWindows) {
                        tool.output.showWarningBadge("Binary data in clipboard", "The binary data you copied contains a NUL character at position {0}. The rest of the data might have been lost.".format(nulCharacterAt));
                    }
                }
                else tool.output.showStatus("can't copy");
        ...
        if (src.indexOf("data:") === 0) {
            ...
            xhttp.onreadystatechange = function() {
                ...
                // CORS Error, forward to backend
                    ...
                    imageToBlob("/get-external-file?url=" + src, callback);
                }
            }
                ...
                var getExternalText = function(url, callback) {
                    ...
                    xhttp.onreadystatechange = function() {
                        if (this.readyState == 4 && this.status == 200) {
                            ...
                            getExternalText("/get-external-file?url=" + url, callback);
                        }
                    }
...
//            tool.resetErrorsOnConvert && tool.output.hideError();
    ...
    tool.options = {
        ...
        get: function(source) {
            var overridden = tool.sides.getAttribute("data-override-options");
            ...
            if (overridden) {
                var defined = JSON.parse(overridden);
...
function makeScrollablePreviews() {
    var previews = document.querySelectorAll(".preview-scrollable");
    if (previews) for (var i = 0; i < previews.length; i++) {
        scrollablePreview(previews[i]);
    }
}
