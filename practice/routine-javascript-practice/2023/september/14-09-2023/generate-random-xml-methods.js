function insertRandomly(which, relative) {
    var position = getRandomIntInclusive(-1, relative.childNodes.length + 1);
    insertElement(which, relative, position);
}
function insertElement(which, relative, position) {
    if (position < 0) {
        relative.parentNode.insertBefore(which, relative);
    } else if (position > relative.childNodes.length) {
        relative.parentNode.insertBefore(which, relative.nextSibling);
    } else {
        relative.insertBefore(which, relative.childNodes[position]);
    }
}
...
function sentence() {
    var word = [WordDictionary.random()];
    while (chance(0.75))
        word.push(WordDictionary.random());
    return word.join(" ");
}
...
;function json2xml(o) {
    var normalizeContent = function(content) {
        if (typeof Html5Entities !== "undefined")
            return Html5Entities.encode(content);
        else
            return content;
    }
...
;!function() {
    ...
    r.prototype.xml = function(r, c) {
        var a = r.replace(/>\s{0,}</g, "><").replace(/</g, "~::~<").replace(/\s*xmlns\:/g, "~::~xmlns:").replace(/\s*xmlns\=/g, "~::~xmlns=").split("~::~")
          ...
          , t = c ? e(c) : this.shift;
        for (n = 0; n < s; n++)
            a[n].search(/<!/) > -1 ? (i += t[l] + a[n],
            p = !0,
            (a[n].search(/-->/) > -1 || a[n].search(/\]>/) > -1 || a[n].search(/!DOCTYPE/) > -1) && (p = !1)) : a[n].search(/-->/) > -1 || a[n].search(/\]>/) > -1 ? (i += a[n],
            p = !1) : /^<\w/.exec(a[n - 1]) && /^<\/\w/.exec(a[n]) && /^<[\w:\-\.\,]+/.exec(a[n - 1]) == /^<\/[\w:\-\.\,]+/.exec(a[n])[0].replace("/", "") ? (i += a[n],
            p || l--) : a[n].search(/<\w/) > -1 && -1 == a[n].search(/<\//) && -1 == a[n].search(/\/>/) ? i = i += p ? a[n] : t[l++] + a[n] : a[n].search(/<\w/) > -1 && a[n].search(/<\//) > -1 ? i = i += p ? a[n] : t[l] + a[n] : a[n].search(/<\//) > -1 ? i = i += p ? a[n] : t[--l] + a[n] : a[n].search(/\/>/) > -1 ? i = i += p ? a[n] : t[l] + a[n] : a[n].search(/<\?/) > -1 ? i += t[l] + a[n] : a[n].search(/xmlns\:/) > -1 || a[n].search(/xmlns\=/) > -1 ? i += t[l] + a[n] : i += a[n];
        return "\n" == i[0] ? i.slice(1) : i
    }
    ,
    ...
    r.prototype.css = function(r, c) {
        var a = r.replace(/\s{1,}/g, " ").replace(/\{/g, "{~::~").replace(/\}/g, "~::~}~::~").replace(/\;/g, ";~::~").replace(/\/\*/g, "~::~/*").replace(/\*\//g, "*/~::~").replace(/~::~\s{0,}~::~/g, "~::~").split("~::~")
          ...
          , n = c ? e(c) : this.shift;
        for (i = 0; i < s; i++)
            /\{/.exec(a[i]) ? l += n[p++] + a[i] : /\}/.exec(a[i]) ? l += n[--p] + a[i] : (/\*\\/.exec(a[i]),
            l += n[p] + a[i]);
        return l.replace(/^\n{1,}/, "")
    }
    ,
    r.prototype.sql = function(r, s) {
        ...
        N = 0; N < l; N++) {
            ...
            /\s{0,}\(\s{0,}SELECT\s{0,}/.exec(i[N]) ? E += o[++n] + i[N] : /\'/.exec(i[N]) ? (g < 1 && n && n--,
            E += i[N]) : (E += o[n] + i[N],
            g < 1 && n && n--)
        }
        return E = E.replace(/^\n{1,}/, "").replace(/\n{1,}/g, "\n")
